...
func GetTblcounter(c *fiber.Ctx) error {
	var items []models.Tblcounter
...
func CreateTblcounter(c *fiber.Ctx) error {
	var item models.Tblcounter
...
func UpdateTblcounter(c *fiber.Ctx) error {
	id := c.Params("id")
	var item models.Tblcounter
	item.Timestamp = time.Now()
...
func DeleteTblcounter(c *fiber.Ctx) error {
	id := c.Params("id")
	if err := c.Locals("db").(*gorm.DB).Delete(&models.Tblcounter{}, id).Error; err != nil {
